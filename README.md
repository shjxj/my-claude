...
| 脚本 | 用途 | 运行方式 |
|------|------|----------|
| `scripts/install.sh` | 核心能力一键安装 + 升级（幂等） | `bash scripts/install.sh` |

> **覆盖率结论**：22/22 项能力由 10 插件 + 7 Skill + 13 MCP 全部覆盖。
...
**分场景模板**（`scripts/claude/templates/`，8 套）：
...
| **安装/验证** | `bash scripts/install.sh`（一键安装 + 升级）|
